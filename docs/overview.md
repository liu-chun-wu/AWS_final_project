...
3. **EC2 Jenkins** (`scripts/jenkins/41-setup-jenkins-ec2.sh`) – provision a long-lived Jenkins host that reuses those Jenkinsfiles and authenticates via the instance profile (defaults to t3.medium).
...
|---------|---------------|-------------|----------|
| `Jeffery` | Auto CI (webhook) + manual CD | Manual `flask-cd` deploy (demo-backend) | Daily development & testing |
...
- `jenkins-pipeline-setting/` – Jenkinsfile-CI and Jenkinsfile-CD; Jenkins always pulls these from Git.
...
- AWS CLI v2 + SAM CLI configured with a Learner Lab profile (us-east-1) – the Jenkinsfiles also self-install AWS/SAM CLI on agents if missing.
